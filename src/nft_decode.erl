...
decode_expr_attrs(Attrs) ->
    Name = case lists:keyfind(?NFTA_EXPR_NAME, 1, Attrs) of
        {_, N} -> strip_null(N);
        false -> <<"unknown">>
    end,
    Data = case lists:keyfind(?NFTA_EXPR_DATA, 1, Attrs) of
        {_, nested, D} -> D;
...
decode_expr(<<"meta">>, Data) ->
    Key = case lists:keyfind(2, 1, Data) of
...
decode_expr(<<"cmp">>, Data) ->
    Op = case lists:keyfind(2, 1, Data) of
        {2, <<O:32/big>>} -> cmp_op_name(O);
        _ -> <<"?">>
    end,
    Value = case lists:keyfind(3, 1, Data) of
        {3, nested, VAttrs} ->
...
decode_expr(<<"payload">>, Data) ->
    Base = case lists:keyfind(2, 1, Data) of
        {2, <<B:32/big>>} -> B;
        _ -> -1
    end,
    Offset = case lists:keyfind(3, 1, Data) of
        {3, <<O:32/big>>} -> O;
        _ -> -1
    end,
    Len = case lists:keyfind(4, 1, Data) of
        {4, <<L:32/big>>} -> L;
        _ -> 0
    end,
...
decode_expr(<<"counter">>, Data) ->
    Bytes = case lists:keyfind(1, 1, Data) of
        {1, <<B:64/big>>} -> B;
        _ -> 0
    end,
    Pkts = case lists:keyfind(2, 1, Data) of
        {2, <<P:64/big>>} -> P;
        _ -> 0
    end,
...
decode_expr(<<"lookup">>, Data) ->
    Set = case lists:keyfind(1, 1, Data) of
        {1, S} -> strip_null(S);
        _ -> <<"?">>
    end,
    #{type => lookup, set => Set};
...
decode_expr(<<"log">>, Data) ->
    Prefix = case lists:keyfind(2, 1, Data) of
        {2, P} -> strip_null(P);
        _ -> <<>>
    end,
...
decode_verdict(Attrs) ->
    VerdictData = case lists:keyfind(2, 1, Attrs) of
        {2, nested, VD} -> VD;
        {2, VBin} when is_binary(VBin) -> nfnl_attr:decode(VBin);
        _ -> []
    end,
    Code = case lists:keyfind(1, 1, VerdictData) of
        {1, <<C:32/big>>} -> C;
        _ -> -1
    end,
...
build_description([#{type := ct, key := <<"state">>},
                   #{type := bitwise},
                   #{type := cmp} | Rest], Acc) ->
    build_description(Rest, [<<"ct state established,related">> | Acc]);

%% meta l4proto + cmp → protocol match
build_description([#{type := meta, key := <<"l4proto">>},
                   #{type := cmp, value := Value} | Rest], Acc) ->
    Proto = case Value of
        <<6>> -> <<"tcp">>;
        <<17>> -> <<"udp">>;
        <<1>> -> <<"icmp">>;
...
build_description([#{type := meta, key := <<"nfproto">>},
                   #{type := cmp} | Rest], Acc) ->
    build_description(Rest, Acc);

%% meta iif + cmp → interface match
build_description([#{type := meta, key := <<"iif">>},
                   #{type := cmp, value := <<1:32/native>>} | Rest], Acc) ->
...
build_description([#{type := payload, base := 2, offset := 2, len := 2},
                   #{type := cmp, value := <<Port:16/big>>} | Rest], Acc) ->
    build_description(Rest, [<<"dport ", (integer_to_binary(Port))/binary>> | Acc]);
build_description([#{type := payload, base := 2, offset := 0, len := 2},
                   #{type := cmp, value := <<Port:16/big>>} | Rest], Acc) ->
    build_description(Rest, [<<"sport ", (integer_to_binary(Port))/binary>> | Acc]);
build_description([#{type := payload, base := 1, offset := 12, len := 4},
                   #{type := cmp, value := <<A, B, C, D>>} | Rest], Acc) ->
...
-spec cmp_op_name(non_neg_integer()) -> binary().
